...
**IMPORTANT**: This project uses **95% io.net Intelligence Platform** as the primary AI engine, with only 5% legacy Claude usage for basic summaries.

#### io.net Models Used:

- **`meta-llama/Llama-3.3-70B-Instruct`**: Primary model for enhanced scoring, risk assessment, TAO Q&A
- **`deepseek-ai/DeepSeek-R1`**: Specialized for trend prediction and comparative analysis
- **`Meta-Llama-3.1-8B-Instruct-Turbo`**: Fast inference for real-time responses

#### Legacy Claude Usage (5%):

- Only used in base `ScoreAgent` class for generating basic AI summaries
- Being phased out in favor of io.net Intelligence
...
class IONetClient:
    ...
    async def generate_summary(self, data: Dict) -> str:
        ...
        response = self.client.chat.completions.create(
            model=self.models['general'],  # Using io.net Llama model
...
st.sidebar.markdown("Powered by io.net Intelligence Platform (95%)")
...
with tab2:
    st.title("AI Subnet Assistant (io.net Intelligence)")
    user_input = st.text_input("Ask about any subnet:")
    if user_input:
        # Call io.net AI for response using Llama-3.3-70B model
        st
```
